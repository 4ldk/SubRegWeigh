...
def get_tfidf(subwords, top_subword=None, tfidf=False):
    if top_subword is not None:
        subword = [s[0] for s in subwords] + [top_subword]
    else:
        subword = [s[0] for s in subwords]
    if tfidf:
...
def get_subword_sequences(tokenizer, sentence, n=100, out=10, method="random", tfidf=False):

    if method == "random":
        selected_subwords = [tokenizer.tokenizeSentence(sentence) for _ in range(out)]
    else:
        subwords = [tokenizer.tokenizeSentence(sentence) for _ in range(n)]
        tokenizer.const_tokenize()
        top_subword = tokenizer.tokenizeSentence(sentence)[0]
